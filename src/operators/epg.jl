...
struct ConfigurationStates{T,Ns,M} <: AbstractConfigurationStates{T}
    matrix::MMatrix{3,Ns,T,M}
end

Base.size(Ω::ConfigurationStates) = size(Ω.matrix)
Base.getindex(Ω::ConfigurationStates, i::Int) = Ω.matrix[i]
Base.getindex(Ω::ConfigurationStates, I::Vararg{Int,N}) where {N} = Ω.matrix[I...]
Base.setindex!(Ω::ConfigurationStates, v, i::Int) = setindex!(Ω.matrix, v, i)
Base.setindex!(Ω::ConfigurationStates, v, I::Vararg{Int,N}) where {N} = setindex!(Ω.matrix, v, I...)
# Base.size(Ω::ConfigurationStates, dim) = size(Ω.matrix, dim)
# Base.getindex(Ω::ConfigurationStates, inds...) = Ω.matrix[inds...]
# Base.setindex!(Ω::ConfigurationStates, v, inds...) = (Ω.matrix[inds...] = v)
# Base.view(Ω::ConfigurationStates, inds...) = view(Ω.matrix, inds...)
# Base.ndims(Ω::ConfigurationStates) = ndims(Ω.matrix)
# Base.ndims(::Type{ConfigurationStates{T}}) where {T} = ndims(T)
# Base.copyto!(Ω::ConfigurationStates, bc::Base.Broadcast.Broadcasted) = copyto!(Ω.matrix, bc)
# Base.length(Ω::ConfigurationStates) = length(Ω.matrix)
# Base.sum(Ω::ConfigurationStates) = sum(Ω.matrix)
# Base.iterate(Ω::ConfigurationStates, state=1) = state > length(Ω) ? nothing : (Ω[state], state + 1)

export ConfigurationStates
...
"""
    excite!(Ω::AbstractConfigurationStates, RF::Complex, p::AbstractTissueParameters)
...
"""
    excite!(Ω::AbstractConfigurationStates, RF::T, p::AbstractTissueParameters) where T<:Union{Real, Quantity{<:Real}}
...
"""
    invert!(Ω::AbstractConfigurationStates, p::AbstractTissueParameters)

Invert `Z`-component of states of all orders. *Assumes fully spoiled transverse magnetization*.
"""
@inline function invert!(Ω::AbstractConfigurationStates, p::AbstractTissueParameters)
    # inversion angle
    θ = π
    hasB₁(p) && (θ *= p.B₁)
    Z(Ω) .*= cos(θ)
