...
@inline function off_resonance_rotation(state::S, Δt::T, p::AbstractTissueParameters) where {S<:Union{Isochromat,AbstractConfigurationStates},T}
    if hasB₀(p)
        θ = π * Δt * p.B₀ * 2
        return exp(im * θ)
    else
...
@inline E₁(state::S, Δt, T₁) where {S<:Union{Isochromat,AbstractConfigurationStates}} = exp(-Δt * inv(T₁))
@inline E₂(state::S, Δt, T₂) where {S<:Union{Isochromat,AbstractConfigurationStates}} = exp(-Δt * inv(T₂))
